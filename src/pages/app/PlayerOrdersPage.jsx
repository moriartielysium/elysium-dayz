...
export default function PlayerOrdersPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await api(`player/orders?slug=${encodeURIComponent(slug)}`);
        if (!active) return;
  ...
  return (
    <PlayerLayout title="Заказы" subtitle="История покупок" nav={nav}>
          ...
          {orders.map((item) => (
            <div key={item.id} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right text-sm text-zinc-300">{formatNumber(item.pricePaid)}</div>
              </div>
              <div className="mt-2 text-xs text-zinc-500">Создан: {item.createdAt || "—"}</div>
            </div>
          ))}

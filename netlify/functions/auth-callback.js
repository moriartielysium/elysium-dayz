...
const { createSession, getStateFromCookies, clearStateCookie } = require("./_lib/session");
...
exports.handler = async (event) => {
  try {
    ...
    if (!code || !state || !savedState || state !== savedState) {
      return redirect(`${config.siteUrl}/?error=oauth_state`, [clearStateCookie()]);
    }
    const tokenData = await exchangeCodeForToken(code);
    const user = await getCurrentUser(tokenData.access_token);
    const sessionCookie = await createSession(user, tokenData);
    return redirect(`${config.siteUrl}/dashboard.html`, [clearStateCookie(), sessionCookie]);
  } catch (error) {

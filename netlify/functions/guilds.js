...
exports.handler = async (event) => {
  try {
    ...
    if (!session.accessToken) return unauthorized("Missing Discord access token");
    const userGuilds = await getUserGuilds(session.accessToken);
    const manageableGuilds = userGuilds.filter((guild) => hasManageAccess(guild.permissions));
    const botGuildIdsResult = await query(`select guild_id from bot_guilds`);
    const botGuildIds = new Set(botGuildIdsResult.rows.map((row) => row.guild_id));
